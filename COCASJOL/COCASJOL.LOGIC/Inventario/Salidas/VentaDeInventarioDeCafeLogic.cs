...
{
    public class VentaDeInventarioDeCafeLogic
    {
        public VentaDeInventarioDeCafeLogic() { }
            ...
            DateTime FECHA_MODIFICACION)
        {
            try
            {
                using (var db = new colinasEntities())
                {
                    var query = from v in db.ventas_inventario_cafe.Include("socios").Include("clasificaciones_cafe")
                                where
                                ...
                                (VENTAS_INV_CAFE_CANTIDAD_LIBRAS == -1 ? true : v.VENTAS_INV_CAFE_PRECIO_LIBRAS.Equals(VENTAS_INV_CAFE_CANTIDAD_LIBRAS)) &&
            ...
            try
            {
                using (var db = new colinasEntities())
                {
                    using (var scope1 = new TransactionScope())
                    {
                        ...
                        venta_cafe.SOCIOS_ID = SOCIOS_ID;
                        ...
                        inventariodecafelogic.InsertarTransaccionInventarioDeCafeDeSocio(venta_cafe, db);
